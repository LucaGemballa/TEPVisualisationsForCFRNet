...
def pdist2(X,Y):
    ...
    C = -2*X.dot(Y.T).astype(float) 
    nx = np.sum(np.square(X),1,keepdims=True).astype(float) 
    ny = np.sum(np.square(Y),1,keepdims=True).astype(float) 
    D = (C + ny.T) + nx
...
def cf_nn(x, t):
    ...
    Itcho = random.sample(It.tolist(),1000)
    Iccho = random.sample(Ic.tolist(),200)

    x_c = x[Iccho]
    x_t = x[Itcho]
    #for our case this gets very very very large 
    D = pdist2(x_c, x_t)
...
def evaluate_cont_ate(predictions, data, i_exp, I_subset=None,
    ...
    # eff ist der gemessene Treatment Effekt 
    """if(mu1 is not None):
        eff = mu1-mu0
    else:
        eff = np.ones((yf_p.shape[0], 1))"""
    ...
    """if(ycf is not None):
        ...
        rmse_cfact= np.ones((ycf_p.shape[0], 1))"""

    #both are eff
    eff_pred = ycf_p - yf_p;
    eff_pred[t>0] = -eff_pred[t>0]; 
    
    #print(np.shape(ycf), np.shape(yf), np.shape(ycf_p))
    #print(np.shape(ycf_p))
    ...
    """return {'ate_pred': ate_pred, 'att_pred': att_pred,
            ...
            'pehe': pehe, 'rmse_ite': rmse_ite, 'pehe_nn': pehe_appr, 'iteff_pred': eff_pred,
            'ycf_p': ycf_p, 'yf_p': yf_p, 'treatment': t,'features': x, 'real_eff': eff}
            #'policy_value': policy_value, 'policy_curve': policy_curve}"""
    return {'iteff_pred': eff_pred,
            'ycf_p': ycf_p, 'yf_p': yf_p, 'treatment': t,'features': x}
            #'policy_value': policy_value, 'policy_curve': policy_curve}
    ...
    for i_out in range(n_outputs):
        ...
        for i_rep in range(n_rep):
            ...
            if multiple_exps:
                i_exp = i_rep
                if validation:
                    nn_t, nn_c = cf_nn(data['x'][I_valid_rep,:], data['t'][I_valid_rep])
                else:
                    nn_t, nn_c = cf_nn(data['x'], data['t'])
            else:
                i_exp = 0
