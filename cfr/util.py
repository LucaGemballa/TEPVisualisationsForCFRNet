...
# replace app.flags with tf.compat.v1.flags
...
def load_data(fname):
    ...
    if fname[-3:] == 'npz':
        ...
        for k in data_in:
            print(data_in[k].shape, k )

        data = {'x': data_in['arr_0'], 't': data_in['arr_1'], 'yf': data_in['arr_2']}
        try:
...
def wasserstein(X,t,p,lam=10,its=10,sq=False,backpropT=False):
    ...
    # had to be gather_nd instead of gather
    Xc = tf.gather(X,ic)
    Xt = tf.gather(X,it)
    #tf.cast(tf.shape(Xc)[0],tf.float32)
    nc = tf.to_float(tf.shape(Xc)[0])
    nt = tf.to_float(tf.shape(Xt)[0])
